    ...
    simatabla += "<tr>";
      for (let j = 0; j < 3; j++) {
        simatabla += "<td>";
          ...
          randomkep = randompic();
          if (randomkep == 9) {
            simatabla += `<img src="img1/ran9.jpg" id="ran${randomkep}">`;
          } else {
              simatabla +=`<img src="img1/ran${randomkep}bw.jpg" class="dropzone" id="ran${randomkep}">`;
          }
          simatabla += "</td>";
      }
      simatabla += "</tr>";
      ...
      for (let j = 0; j < 3; j++) {
        simatabla += "<td>";
          ...
          if (randomkep == 9) {
              ...
              simatabla +=`<img src="img1/ran${randomkep}.jpg" class="draggable" id="ran${randomkep}">`;
          }
          simatabla += "</td>";
      }
      simatabla += "</tr>";
...
function delegate(parent, type, selector, fn) {
  ...
  function onPointerUp(e) {
    ...
    if (activeDropZone) {
      if (egerfole.id == e.target.id && difficulty == "easy") {
        ...
        activeDropZone.parentNode.innerHTML = `<img id="${activeDropZone.id}" src="img1/${activeDropZone.id}.jpg">`;
        activeDropZone.classList.remove('active');
      ...
      if (egerfole.id != e.target.id && difficulty == "hard") {
        activeDropZone.appendChild(e.target);
        activeDropZone.parentNode.innerHTML = `<img id="${activeDropZone.id}" src="img1/${e.target.id}.jpg">`;
        activeDropZone.classList.remove('active');
        activeDropZone.classList.remove('dropzone');
        activeDropZone = undefined;
        haasikergarantalthrd();
  ...
  function haasikergarantalthrd() {
    ...
    elhelyezett++;
    if (valtozo1.getAttribute("src") == "img1/ran1.jpg" && valtozo2.getAttribute("src") == "img1/ran2.jpg" && valtozo3.getAttribute("src") == "img1/ran3.jpg" && valtozo4.getAttribute("src") == "img1/ran4.jpg" &&
      valtozo5.getAttribute("src") == "img1/ran5.jpg" && valtozo6.getAttribute("src") == "img1/ran6.jpg" && valtozo7.getAttribute("src") == "img1/ran7.jpg" && valtozo8.getAttribute("src") == "img1/ran8.jpg") {
      gyozelem();
    }
    else {
      if (elhelyezett == 8) {
        ...
        for (let i = 0; i < 8; i++) {
          let segged = $(`#ran${i+1}`);
          console.log(segged);
          if (segged.getAttribute("src") != `img1/ran${i + 1}.jpg`) {
            segged.src = `img1/ran${i + 1}bw.jpg`;
            segged.className = "dropzone";
            second.innerHTML += `<tr><td><img src="img1/ran${i + 1}.jpg" class="draggable" id="ran${i + 1}"></td></tr>`;
