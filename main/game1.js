    ...
    simatabla += "<tr>";
      for (let j = 0; j < 3; j++) {
        simatabla += "<td>";
          ...
          randomkep = randompic();
          if (randomkep == 9) {
            simatabla += `<img src="img2/ran4.png" id="ran${randomkep}">`;
          } else {
              simatabla +=`<img src="img2/ran${randomkep}bw.png" class="dropzone" id="ran${randomkep}">`;
          }
          simatabla += "</td>";
      }
      simatabla += "</tr>";
      ...
      for (let j = 0; j < 3; j++) {
        simatabla += "<td>";
          ...
          if (randomkep == 9) {
              ...
              simatabla +=`<img src="img2/ran${randomkep}.png" class="draggable" id="ran${randomkep}">`;
          }
          simatabla += "</td>";
      }
      simatabla += "</tr>";
...
function delegate(parent, type, selector, fn) {
  ...
  function onPointerUp(e) {
    ...
    if (activeDropZone) {
      if (egerfole.id == e.target.id && difficulty == "easy") {
        ...
        activeDropZone.parentNode.innerHTML = `<img id="${activeDropZone.id}" src="img2/${activeDropZone.id}.png">`;
        activeDropZone.classList.remove('active');
      ...
      if (egerfole.id != e.target.id && difficulty == "hard") {
        activeDropZone.appendChild(e.target);
        activeDropZone.parentNode.innerHTML = `<img id="${activeDropZone.id}" src="img2/${e.target.id}.png">`;
        activeDropZone.classList.remove('active');
        activeDropZone.classList.remove('dropzone');
        activeDropZone = undefined;
        haasikergarantalthrd();
  ...
  function haasikergarantalthrd() {
    ...
    elhelyezett++;
    if (valtozo1.getAttribute("src") == "img2/ran1.png" && valtozo2.getAttribute("src") == "img2/ran2.png" && valtozo3.getAttribute("src") == "img2/ran3.png" && valtozo4.getAttribute("src") == "img2/ran4.png" &&
      valtozo5.getAttribute("src") == "img2/ran5.png" && valtozo6.getAttribute("src") == "img2/ran6.png" && valtozo7.getAttribute("src") == "img2/ran7.png" && valtozo8.getAttribute("src") == "img2/ran8.png") {
      gyozelem();
    }
    else {
      if (elhelyezett == 8) {
        ...
        for (let i = 0; i < 8; i++) {
          let segged = $(`#ran${i+1}`);
          console.log(segged);
          if (segged.getAttribute("src") != `img2/ran${i + 1}.png`) {
            segged.src = `img2/ran${i + 1}bw.png`;
            segged.className = "dropzone";
            second.innerHTML += `<tr><td><img src="img2/ran${i + 1}.png" class="draggable" id="ran${i + 1}"></td></tr>`;
